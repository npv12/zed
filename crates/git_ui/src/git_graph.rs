...
use ui::{
    ...
    PopoverMenu, PopoverMenuHandle, RedistributableColumnsState, ScrollableHandle, Table,
    TableInteractionState, TableRenderContext, TableResizeBehavior, ToggleState, Tooltip,
    WithScrollbar, bind_redistributable_columns, prelude::*, redistribute_hidden_fractions,
            ...
            .map(|chip| {
                if is_head {
                    chip.bg_color(accent_color.opacity(0.25))
                        .border_color(accent_color.opacity(0.8))
                } else if is_tag {
                    chip.bg_color(accent_color.opacity(0.12))
                        .border_color(accent_color.opacity(0.4))
                } else if is_stash {
                    chip.bg_color(accent_color.opacity(0.18))
                        .border_color(accent_color.opacity(0.4))
                } else {
                    ...
                    div()
                        ...
                        .child(
                            h_flex()
                                ...
                                .overflow_hidden()
                                .children({
                                    let visible_ref_names: Vec<_> = commit
                                        .data
                                        .ref_names
                                        .iter()
                                        .filter(|name| self.is_visible_ref_name(name.as_ref()))
                                        .collect();
                                    let display_ref_names: Vec<_> = visible_ref_names
                                        .iter()
                                        .map(|name| {
                                            Self::display_ref_name(
                                                name,
                                                commit.data.sha,
                                                &stash_entries,
                                            )
                                        ...
                                        .collect();
                                    (!display_ref_names.is_empty()).then(|| {
            ...
            move |bounds: Bounds<Pixels>, _: (), window: &mut Window, cx: &mut App| {
                ...
                window.paint_layer(bounds, |window| {
                    ...
                    for (visible_row_idx, (entry_idx, _, _)) in rows.iter().enumerate() {
                        ...
                        let is_context_menu_target = context_menu_target_index == Some(*entry_idx);
...
impl Render for GitGraph {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content = if visible_commit_count == 0 {
            ...
            let show_graph_column =
                !is_path_history && graph_visible && !self.filter_search_matches();
                    ...
                    v_flex()
                        ...
                        .child({
                            ...
                            let commits_table = Table::new(4)
                                ...
                                .hide_row_borders()
                                .hide_row_hover()
                                .width_config(self.table_column_width_config(window, cx))
                                ...
                                .map_row(move |(index, row), window, cx| {
                                    ...
                                    let is_context_menu_target =
                                        context_menu_target_index == Some(entry_index);
...
impl CreateBranchAtCommitModal {
    ...
    fn confirm(&mut self, _: &menu::Confirm, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |_, cx| match rx.await {
            ...
            Ok(Err(error)) => log::error!("failed to create branch: {error:?}"),
            Err(_) => log::error!("branch creation was canceled"),
        })
        .detach();
        cx.emit(DismissEvent);
    }
...
mod persistence {
    ...
    pub fn serialize_log_source_value(log_source: &LogSource) -> Option<String> {
        match log_source.base_source() {
            LogSource::All | LogSource::Branches(_) => None,
